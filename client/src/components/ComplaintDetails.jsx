...
const SERVER_BASE = API_BASE.replace("/api/", "/");
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center z-50">
      <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] shadow-xl relative flex flex-col overflow-hidden border border-gray-200">
        ...
        <div className="px-6 pt-6">
          <h2 className="text-3xl font-semibold text-gray-900 pr-10">
            {complaint.Title}
          </h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-6">
          ...
          {mediaFiles.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {mediaFiles.map((media, idx) => {
                  const imageUrl = `${SERVER_BASE}uploads/${media.ImageURL}`
                  // `${import.meta.env.VITE_API_BASE_URL
                  // // }uploads/${media.fileName}`;
                  // }uploads/${media.ImageURL}`;
